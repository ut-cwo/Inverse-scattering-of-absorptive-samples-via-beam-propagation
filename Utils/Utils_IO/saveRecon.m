...
%                    Save utility file for mainMSBP.m                     %
%                                                                         %
%                                                                         %
...
%% saveRecon:   Save reconstruction variables, a parameter file, metadata, 
%%              and plain text files of the code.
% Arguments:
...
%       SampleDir/Reconstructions/[ID]_Recon/parFile/
...
%       documented in the file itself.
    ...
    parFileDir  = fullfile(reconDir, 'parFile');
    ...
    % Save reconstruction parameters into parFile
    if isfield(extraData, 'r')
        saveParFile(parFileDir, extraData.r)
    end
...
function saveParFile(parFileDir, reconPars)
%% saveVarsMat: Gather and save volume as mat
% Arguments:
%   parFileDir  -   Path to parFile directory
%   reconPars   -   Recosntruction parameters
    matFile = fullfile(parFileDir, 'parFile.mat');
    try
        reconPars   = rmfield(reconPars, {'varsToSave','cost', ...
                                          'tLoopCheck','tTotCheck'} );
        save(matFile, '-struct', 'reconPars');
    catch
        save(matFile, '-struct', 'reconPars');
    end
end
