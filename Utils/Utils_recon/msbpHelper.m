...
%               Algorithmic helper function for mainMSBP.m                %
%                                                                         %
% This code is a consolidation of the standard MSBP main loop from        %
% previous works. Parts of the algorithm have been updated to increase    %
% execution speed and to implement the necessary changes for complex-     %
% valued reconstructions.                                                 %
%                                                                         %
...
p.N         = single(r.patchFOV+2*r.pdar);      % Number of pixels in padded FOV
...
% Filter NA Crop to remove hard boundary
lpf            = fspecial('gaussian', [5 5]);
p.NA_crop      = imfilter(p.NA_crop, lpf, 'replicate');
...
if r.viewUpdates
    ...
    MSBP_progview(real(reconObj), 1, r.plotRangeRe, r.cost, 0)
    MSBP_progview(imag(reconObj), 2, r.plotRangeIm, r.cost, 0)
    drawnow();
end
...
for iter = 1:r.maxIter
    ...
    for angleScan = 1:length(seq)
        ...
        if mod(totAngles,r.regFreq)==0
            % Apply positivty constraint to real-valued refractive index
            if r.positivityCon
                reconObj    = complex(max(real(reconObj),0),imag(reconObj));
            end
            ...
            [proxObj, dualRe]   = proxTV_3d(    real(reconObj), ...
                                                r.regParamRe, ...
                                                r.tvIters, r.warmStart, ...
                                                r.tvItersWarm, dualRe);
            if r.useComplex
                [tmp, dualIm]   = proxTV_3d(    imag(reconObj), ...
                                                r.regParamIm, ...
                                                r.tvIters, r.warmStart, ...
            ...
            if r.nesterov
                ...
                if iter>1 && r.cost(iter)>r.cost(iter-1)
                    % Reset momentum
                    t_k    = 1;
                end
            else
    ...
    if r.viewUpdates
        % Update Display
        MSBP_progview(real(reconObj), 1, r.plotRangeRe, r.cost, iter)
        MSBP_progview(imag(reconObj), 2, r.plotRangeIm, r.cost, iter)
        drawnow();
    end
